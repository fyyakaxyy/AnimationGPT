...
def plot_3d_motion(save_path, kinematic_tree, joints, title, figsize=(10, 10), fps=120, radius=4):
    def init(fig, ax):
        ...
        formatted_title = re.sub(r'[,.]', r'\g<0>\n', title)  # 在逗号和句号处换行
        fig.suptitle(formatted_title, fontsize=12)
        ax.grid(b=False)
...
def process_npy_files(src_dir, tgt_ani_dir, kinematic_chain):
    ...
    for npy_file in tqdm(npy_files):
        ...
        txt_file_name = npy_filename.replace("out", "in")    
        txt_file = os.path.join(src_dir, f"{txt_file_name}.txt")
        if not os.path.exists(txt_file):
            raise FileNotFoundError(f"Corresponding text file not found: {txt_file}")

        text_content = get_text_from_txt(txt_file)
        ...
        if os.path.exists(save_path):
            continue
        plot_3d_motion(save_path, kinematic_chain, data, title=text_content, fps=30, radius=4)
...
if __name__ == "__main__":
    ...
    process_npy_files(src_dir, tgt_ani_dir, kinematic_chain)
