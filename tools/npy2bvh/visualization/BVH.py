...
# 示例数据，替换成你自己的数据
data = {
    'rotations': np.random.rand(100, 10, 3),  # 示例旋转数据，形状为 (帧数, 关节数, 3)
    'positions': np.random.rand(100, 10, 3),  # 示例位置数据，形状为 (帧数, 关节数, 3)
    'offsets': np.zeros((10, 3)),  # 示例关节偏移，形状为 (关节数, 3)
    'parents': np.array([0, 1, 2, 2, 0, 5, 6, 7, 8, 0]),  # 示例关节父级关系
    'names': ['Joint{}'.format(i) for i in range(10)],  # 示例关节名称
    'frametime': 0.0333333  # 示例每一帧的时间间隔
}

# 创建 Animation 对象
animation = Animation(data['rotations'], data['positions'], np.zeros_like(data['positions']),
                      data['offsets'], data['parents'], data['names'], data['frametime'])

# 将 Animation 对象保存为 BVH 文件
write_bvh(data['parents'], data['offsets'], data['rotations'], data['positions'],
          data['names'], data['frametime'], 'xyz', 'output.bvh')